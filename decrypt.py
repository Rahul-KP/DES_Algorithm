...
def cipher_16(IP):
    ...
    print(hex(bitarray.bin_to_dec(L))[2:],hex(bitarray.bin_to_dec(R))[2:])
    for i in range(16):
        Ln = R.copy() #Rn-1 = R
        ...
        s_output.permute(P)
        L.XOR(s_output)
        print(hex(bitarray.bin_to_dec(Ln))[2:],hex(bitarray.bin_to_dec(L))[2:],hex(bitarray.bin_to_dec(keys_16[i]))[2:])
        R = L
        L = Ln
    return R,L
...
cipher_text = bitarray(input('Enter cipher text in hex: '))
key = bitarray(input('Enter key in hex : '))

cipher_text.zero_padding()
key.zero_padding()

cipher_text_org = cipher_text.copy()
key_org = key.copy()
...
cipher_text.permute(ip_table)
IP_bits = bitarray.split_half(cipher_text)

R16,L16 = cipher_16(IP_bits)
reverse = R16 + L16

reverse.permute(IP_inv)
print(hex(bitarray.bin_to_dec(reverse.bits))[2:])
